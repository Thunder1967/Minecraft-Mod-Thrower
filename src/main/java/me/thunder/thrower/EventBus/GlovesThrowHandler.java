...
public class GlovesThrowHandler {
    ...
    public static void onRightClickItem(PlayerInteractEvent.RightClickItem event) {
        ...
        if(otherItem.is(ModItems.GLOVES.get()) &&
                handleThrow(player,event.getLevel(),event.getItemStack())){
    ...
    private static boolean handleThrow(Player player,Level level,ItemStack item){
        if (item.isEmpty() || item.is(ModTags.Items.CanNotThrowByGloves)) return false;
            ...
            ThrowEntity(player,level,item,1, true,
                    (a,b,c) -> new PrimedTnt(b, a.getX(), a.getEyeY(), a.getZ(), a));
        }
        else if (item.getItem() instanceof SpawnEggItem ||
                item.is(Items.BUCKET) ||
                item.is(Items.FIRE_CHARGE) ||
                item.is(Items.END_CRYSTAL)) {
            ThrowEntity(player,level,item,1, true, FlyingItem::new);
        }
            ...
            if (!player.getAbilities().instabuild) {
                ...
                item.is(Items.MACE)){
            ThrowEntity(player,level,item,1, false, FlyingTool::new);
            item.shrink(1);
        }
        else if(item.getItem() instanceof BlockItem){
            ThrowEntity(player,level,item,1, true,
                    (a,b,c)->
                            new FlyingBlock(b, a, ((BlockItem)c.getItem()).getBlock().defaultBlockState()));
        }
        else if(item.getItem() instanceof MobNetItem){
            ThrowEntity(player,level,item,1, true, MobNetEntity::new);
        }
        else{
            ThrowEntity(player,level,item,1, true,
                    (a,b,c)->{
    ...
    private static <T extends Entity> void ThrowEntity(Player player, Level level, ItemStack item, double speed, boolean doShrink, resourceFactory<T> factory){
        level.playSound(
                player,
        ...
        if(!level.isClientSide){
            ...
            T thrownEntity = factory.create(player,level,stackCopy);
    ...
    public interface resourceFactory<T extends Entity>{
        T create(Player player,Level level,ItemStack item);
    }
}
